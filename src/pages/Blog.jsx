...
const posts = [
  { title: "How to Build a Placement-ready MERN Portfolio", category: "Career", tags: ["MERN", "Projects"] },
  { title: "Python Skills Every Beginner Should Practice", category: "Python", tags: ["Python", "Basics"] },
  { title: "Mock Interview Checklist for Freshers", category: "Interview", tags: ["HR", "Aptitude"] }
];
          ...
          {posts.map((post) => (
            <GlassCard className="p-6" key={post.title}>
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <span className="rounded-lg bg-white/10 px-3 py-1 text-xs text-white/60" key={tag}>{tag}</span>
                ))}
              </div>
              <p className="mt-5 text-sm text-white/45">Comments enabled when article API is connected.</p>
            </GlassCard>
          ))}
        </div>
      </div>
